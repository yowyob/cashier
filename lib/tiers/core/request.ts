...
export const base64 = (str: string): string => {
    try { return btoa(str); } catch (err) { return Buffer.from(str).toString('base64'); }
};
...
export const getHeaders = async (config: OpenAPIConfig, options: ApiRequestOptions): Promise<Headers> => {
    ...
    const headers = Object.entries({ Accept: 'application/json', ...additionalHeaders, ...options.headers })
        .filter(([_, value]) => isDefined(value))
        .reduce((headers, [key, value]) => ({ ...headers, [key]: String(value) }), {} as Record<string, string>);
    ...
    if (options.body !== undefined) {
        ...
        else if (!isFormData(options.body)) headers['Content-Type'] = 'application/json';
    }
...
export const getRequestBody = (options: ApiRequestOptions): any => {
    if (options.body !== undefined) {
        ...
        else if (isString(options.body) || isBlob(options.body) || isFormData(options.body)) return options.body;
...
export const sendRequest = async (
    ...
    url: string, body: any, formData: FormData | undefined,
    headers: Headers, onCancel: OnCancel
): Promise<Response> => {
    const controller = new AbortController();
    const request: RequestInit = { headers, body: body ?? formData, method: options.method, signal: controller.signal };
    ...
    return new CancelablePromise(async (resolve, reject, onCancel) => {
        try {
            ...
            if (!onCancel.isCancelled) {
                ...
                const result: ApiResult = { url, ok: response.ok, status: response.status, statusText: response.statusText, body: responseHeader ?? responseBody };
