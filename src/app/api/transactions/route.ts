...
export async function GET(request: Request) {
    try {
        ...
        const backendResponse = await fetchBackend(`/api/transactions${search}`, {
            cache: "no-store"
        });
        const body = await readBackendJson(backendResponse);
        if (!backendResponse.ok) {
            return NextResponse.json(
                { error: body?.error || "Failed to load transactions." },
                { status: backendResponse.status }
            );
        }
        const payload = Array.isArray(body) ? body : Array.isArray(body?.data) ? body.data : body?.movements ?? [];
