...
export async function GET(request: Request) {
    try {
        ...
        const backendResponse = await fetchBackend(`/api/cashiers/available${search}`, {
            cache: "no-store"
        });
        ...
        if (!backendResponse.ok) {
            return NextResponse.json(
                { error: body?.error || "Failed to load available cashiers." },
        ...
        return NextResponse.json({ error: error?.message || "Failed to load available cashiers." }, { status: 500 });
    }
}
