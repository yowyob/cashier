...
export async function GET(request: Request) {
    try {
        ...
        const backendResponse = await fetchBackend(`/api/users/admins${search}`, {
            cache: "no-store"
        });
        ...
        if (!backendResponse.ok) {
            return NextResponse.json(
                { error: body?.error || "Failed to load admins." },
...
export async function POST(request: Request) {
    try {
        ...
        const backendResponse = await fetchBackend("/api/users/admins", {
            method: "POST",
        ...
        if (!backendResponse.ok) {
            return NextResponse.json(
                { error: body?.error || "Failed to create admin." },
                { status: backendResponse.status }
            );
        }
